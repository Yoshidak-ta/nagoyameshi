...
public class FavoriteController {
	...
	@PostMapping("/stores/{id}/favorites/register")
	public String register(@PathVariable(name = "id")Integer id, @AuthenticationPrincipal UserDetailsImpl userDetailsImpl, RedirectAttributes redirectAttributes, Model model) {
		Store store = storeRepository.getReferenceById(id);
		User user = userDetailsImpl.getUser();
		favoriteService.register(store, user);
		
		redirectAttributes.addFlashAttribute("seccessMessage", "お気に入りに登録しました。");
	...
	public String delete(@PathVariable(name = "favoriteId")Integer id, RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("seccessMessage", "お気に入り登録を解除しました。");
